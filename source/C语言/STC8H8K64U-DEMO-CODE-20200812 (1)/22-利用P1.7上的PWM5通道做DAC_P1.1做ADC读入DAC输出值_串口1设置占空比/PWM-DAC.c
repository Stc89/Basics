...
从P1.7(PWM5)输出16位的PWM, 输出的PWM经过RC滤波成直流电压送P1.1做ADC并用数码管显示出来.
...
#define PWM2_CCR1 (*(unsigned char volatile xdata *)0xfef5)
#define PWM2_CCR1H (*(unsigned char volatile xdata *)0xfef5) //CCR5 预装载值
#define PWM2_CCR1L (*(unsigned char volatile xdata *)0xfef6)
#define PWM2_ARR (*(unsigned char volatile xdata *)0xfef2)
#define PWM2_ARRH (*(unsigned char volatile xdata *)0xfef2)  //自动重装载高 8 位值
...
void main(void)
{
    ...
    //  ADC初始化
		...
		P_SW2 |= 0x80;
    PWM2_CCER1 = 0x00; //写 CCMRx 前必须先清零 CCERx 关闭通道
    PWM2_CCMR1 = 0x68; //设置 CC1 为 PWM模式1 输出
    PWM2_CCER1 = 0x01; //使能 CC5 通道

    PWM2_ARRH = 2; //设置周期时间
    PWM2_ARRL = 0;
    PWM2_CCR1H = 0;
    PWM2_CCR1L = 128; //设置占空比时间
    PWM2_ENO = 0x01; //使能 PWM5 输出
		PWM2_PS = 0x01;  //高级 PWM 通道 5 输出脚选择位, 0x00:P2.0, 0x01:P1.7, 0x02:P0.0, 0x03:P7.4
    PWM2_BKR = 0x80; //使能主输出
		//PWM2_IER = 0x01; // 使能中断
    PWM2_CR1 |= 0x01; //开始计时
		P_SW2 &= 0x7f;
...
{   
    ...
    {                                   
        P50 = ~P50;
        PWM2_SR1 &=~0X02;    //P5_5=~P5_5;     //读ccr1l清零               
    }  
}
...
{
		P_SW2 |= 0x80;
	  PWM2_CCR1H = (u8)(pwm_value >> 8); //设置占空比时间
    PWM2_CCR1L = (u8)(pwm_value);
		P_SW2 &= 0x7f;
}
