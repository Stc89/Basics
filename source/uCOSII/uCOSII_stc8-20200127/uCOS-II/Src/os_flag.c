...
static  void     OS_FlagBlock(OS_FLAG_GRP *pgrp, OS_FLAG_NODE *pnode, OS_FLAGS flags, INT8U wait_type, INT32U timeout) large reentrant;
static  BOOLEAN  OS_FlagTaskRdy(OS_FLAG_NODE *pnode, OS_FLAGS flags_rdy) large reentrant;
...
OS_FLAGS  OSFlagPendGetFlagsRdy (void) large reentrant
{
    OS_FLAGS      flags;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
    OS_CPU_SR     cpu_sr = 0u;
...
static  void  OS_FlagBlock (OS_FLAG_GRP  *pgrp,
                            ...
                            OS_FLAGS      flags,
                            INT8U         wait_type,
                            INT32U        timeout) large reentrant
{
...
void  OS_FlagInit (void) large reentrant
{
#if OS_MAX_FLAGS == 1u
...
static  BOOLEAN  OS_FlagTaskRdy (OS_FLAG_NODE *pnode,
                                 OS_FLAGS      flags_rdy) large reentrant
{
    OS_TCB   *ptcb;
    BOOLEAN   sched;
